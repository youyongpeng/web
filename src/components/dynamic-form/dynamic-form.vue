<template>
  <el-form class="dynamic-form" :inline="formConfig.inline" :model="value" :label-position="formConfig.labelPosition" :label-width="formConfig.labelWidth" :size='formConfig.size' :status-icon="formConfig.statusIcon">
    <el-row v-if="checkLayoutType === 1" v-for="(pitem, pindex ) in formConfig.formItemList" :key="pindex" >
...
export default {
  ...
  computed: {
    checkLayoutType () {
      let type = 3
      if (this.formConfig.layout === undefined || this.formConfig.layout === false) {
        type = 3
      } else if (this.formConfig.layout !== undefined && this.formConfig.layout === true && this.formConfig.formItemList[0].constructor === Array) {
        type = 1
      } else if (this.formConfig.layout !== undefined && this.formConfig.layout === true) {
        type = 2
      }
      return type
    }
  ...
  methods: {
    ...
    handleInput (val, item) {
      if (item.type === 'datetimerange' || item.type === 'daterange') {
        if (item.names && val && val.length > 0) {
          ...
          this.$emit('input', { ...this.value, [startName]: startTime, [endName]: endTime })
        } else {
          if (item.names) {
            let startName = item.names[0]
            let endName = item.names[1]
            this.$emit('input', { ...this.value, [startName]: null, [endName]: null })
          } else {
    ...
    setDefaultValue () {
      ...
      } else if (this.checkLayoutType === 3 || this.checkLayoutType === 2) {
