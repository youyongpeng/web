...
<template>
    <el-form-item  v-if="item.type === 'datetimerange' || item.type === 'daterange'" :rules="Rules" :label="item.label"  >
      <el-date-picker
        v-model="value"
        :type="item.type"
    ...
    <el-form-item v-else-if="item.type === 'timeselect'" :rules="Rules" :label="item.label" :prop="item.name" >
      <el-time-select
        v-bind="$attrs"
        v-model="value"
        v-on="$listeners"
  ...
  <el-form-item v-else-if="item.type === 'timepicker'" :rules="Rules" :label="item.label" :prop="item.name" >
    <el-time-picker
      v-bind="$attrs"
      v-model="value"
      :is-range="item.isRange"
  ...
  <el-form-item v-else-if="item.type === 'switch'" :rules="Rules" :label="item.label" :prop="item.name" >
    <el-switch
      :disabled="item.disabled"
      v-bind="$attrs"
      v-on="$listeners"
  ...
  <el-form-item v-else-if="item.type === 'inputnumber'" :rules="Rules" :label="item.label" :prop="item.name">
    <el-input-number
      :readonly="item.readonly"
      :disabled="item.disabled"
      v-bind="$attrs"
  ...
  <el-form-item v-else-if="item.type === 'ossfileone'" :label="item.label" >
    ...
    <el-form-item v-else :rules="Rules" :label="item.label" :prop="item.name" >
      ...
      <el-input v-if="item.type==='input'" v-bind="$attrs" v-on="$listeners" :type="item.subtype" :placeholder="item.placeholder" :disabled="item.disabled" :readonly="item.readonly" :autosize="item.autosize"></el-input>
...
export default {
  ...
  created () {
    if (this.item.type === 'datetimerange' && !this.item.names) {
      ...
      if (this.item.isRange !== undefined && this.item.isRange === true && this.item.names === undefined) {
        throw new Error(' timepicker is isRange = true must has names field')
      } else {
        if (this.item.name === undefined && (this.item.isRange === undefined || this.item.isRange === false)) {
