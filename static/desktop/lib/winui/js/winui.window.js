...
layui.define(['layer', 'winui'], function (exports) {
  ...
  WinLayer.prototype.open = function (options) {
    ...
    // 去除最小化按钮
    if (!(options.min === undefined ? this.settings.min : options.min)) { $(windowDom).find('.layui-layer-min').remove() }
    // 去除最大化按钮
    if (!(options.max === undefined ? this.settings.max : options.max)) { $(windowDom).find('.layui-layer-max').remove() }
    // 增加任务项
