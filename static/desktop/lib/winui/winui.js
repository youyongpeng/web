  ...
  layui.define(['jquery', 'layer', 'element', 'form'], function (exports) {
    ...
    Winui.prototype = {
      ...
      init: function (options, callback) {
        ...
        if (options.audioPlay && !layui.sessionData('winuiSession').audio) {
          ...
          if (audio.canPlayType('audio/mp3')) { format = '.mp3' } else if (audio.canPlayType('audio/ogg')) { format = '.wav' }
          if (format) {
        ...
        // 初始化完毕回调
        if (typeof callback === 'function') { callback.call(othis) }
      },
      ...
      resetTaskbar: function (taskbarMode) {
        if (taskbarMode) {
          if (this.settings.taskbarMode === taskbarMode) { return }
      ...
      resetStartSize: function (startSize) {
        if (startSize) {
          if (this.settings.startSize === startSize) { return }
      ...
      renderBg: function () {
        ...
        if (bgSrc) { $('body').css('background-image', 'url(' + bgSrc + ')') }
      },
    ...
    var common = {
      ...
      setData: function (obj, callback) {
        ...
        if (!currOptions.url || !currOptions.method) { return }
        $.ajax({
          url: currOptions.url,
